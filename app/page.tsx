...
export default function Page() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto dark:bg-gray-900 min-h-screen transition-colors">
      ...
      <div className="bg-gray-200 dark:bg-gray-700 p-3 rounded text-center font-semibold mb-6 text-gray-900 dark:text-gray-100">
        Status: {status}
      </div>

      <input value={identity} onChange={e => setIdentity(e.target.value)} className="border p-2 w-full mb-2 rounded dark:bg-gray-800 dark:text-white" />
      <select value={company} onChange={e => setCompany(e.target.value as 'A' | 'B')} className="border p-2 w-full mb-2 rounded dark:bg-gray-800 dark:text-white">
        <option value="A">Company A</option>
        <option value="B">Company B</option>
      </select>
      <input value={target} onChange={e => setTarget(e.target.value)} placeholder="Dial Number e.g. +1856230" className="border p-2 w-full mb-4 rounded dark:bg-gray-800 dark:text-white" />
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Company A — Call History</h2>
          {logs.A.length === 0 ? <p className="text-gray-500 dark:text-gray-400">No calls yet</p> : <ul>{logs.A.map((c: any, i: number) => <li key={i}>{c}</li>)}</ul>}
        </div>
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Company B — Call History</h2>
          {logs.B.length === 0 ? <p className="text-gray-500 dark:text-gray-400">No calls yet</p> : <ul>{logs.B.map((c: any, i: number) => <li key={i}>{c}</li>)}</ul>}
        </div>
      </div>
    </div>
  )
